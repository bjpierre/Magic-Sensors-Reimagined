...
#define VERBOSITY 1 //0 = only to uart, 1 = drops mac to console, 2 = drops mac and csi to serial, 3 = console, no uart
#define CSI_RAW 0
#define RSSI 0
...
void _wifi_csi_cb(void *ctx, wifi_csi_info_t *data) {
    ...
    outprintf("Handle from %02X:%02X:%02X:%02X:%02X:%02X\n", d.mac[0], d.mac[1], d.mac[2], d.mac[3], d.mac[4], d.mac[5]);
#endif
...
#endif
#if CSI_AMPLITUDE
     for (int j = 6; j < 64-5; j++) {
         index += sprintf(&buffer[index],"%.3f,", sqrt(pow(my_ptr[j * 2], 2) + pow(my_ptr[(j * 2) + 1], 2)));
     }
#endif
#if CSI_PHASE
     for (int k = 6; k < 63-5; k++) {
        index += sprintf(&buffer[index],"%.3f,", atan2(my_ptr[k*2], my_ptr[(k*2)+1]));
    }
    index += sprintf(&buffer[index],"%.3f", atan2(my_ptr[(63-5)*2], my_ptr[((63-5)*2)+1]));
#endif
